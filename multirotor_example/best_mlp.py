...
def mlpdistance():
    vel = [9]
    for i in range(72):
        vel.append(clf.predict([[x_coor[i], y_coor[i], z_coor[i], yaw[i]-90, x_coor[i+1], y_coor[i+1], z_coor[i+1], yaw[i+1]-90]])[0])
    return vel
v = mlpdistance()
def move():
    c.takeoffAsync().join()
    ...
    for i in range(73):
        c.moveToPositionAsync(x_coor[i], y_coor[i], z_coor[i], v[i]-1).join()
