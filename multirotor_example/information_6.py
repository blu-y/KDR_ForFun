...
def velocity_distance():
    ...
    for i in range(72):
        ...
        if distance[i] <= 15:
            ...
            final_v[i] -= 2
        # try:
        #     abs(z_coor[i+1] - z_coor[i-1]) < 2 and delta_yaw[i] <15
        #     final_v[i] += 0.8
        # except:
        #     pass
    final_v.append(10)
...
# for i in range(71):
#     print(z_coor[i+1] - z_coor[i], distance[i], yaw[i], delta_yaw[i], delta_yaw[i+1]-delta_yaw[i])
# print(z_coor[71], distance[71], yaw[71], delta_yaw[71])
...
def move():
    ...
    for i in range(73):
        if i >= 64 :
            ...
            c.moveByVelocityAsync(-500*velocity_element[0][i], -500*velocity_element[1][i], -500*velocity_element[2][i], 1)
        #c.rotateToYawAsync(angle_list[i])
        c.rotateToYawAsync(yaw[i]-90)
        time.sleep(time_sleep[i] + 0.02)
move()
...
#print(coefficient[0][0], coefficient[0][1])
...
#print(middle_coor[0][0], middle_coor[1][0][0])
